...
mutate(litters_df, 
       ...
       group = str_to_lower(group)) # Change Con7 to con7
```
...
## ‘arrange’ not that often

``` r
arrange(litters_df, pups_born_alive) # put something in order
```
...
``` r
arrange(litters_df, pups_born_alive, gd0_weight) # first and second in order
```
